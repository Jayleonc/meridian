...
当请求涉及已经部署的 Meridian 运行状态、服务健康、日志、时间、配置、Console 构建或“线上看起来不对”的现象时，优先使用 DevOps MCP 获取事实，再结合代码判断下一步。
...
如果这一步发现 DevOps MCP 暴露的只读能力不够定位问题，可以把补充能力作为本次或后续小切片；但写操作仍必须遵守 DevOps MCP 只读边界，不能临时绕过鉴权和审批设计。
